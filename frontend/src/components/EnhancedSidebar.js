...
const EnhancedSidebar = () => {
  ...
  return (
    <aside className="w-64 bg-gradient-to-b from-gray-900 via-gray-850 to-gray-800 text-white h-screen overflow-y-auto sticky top-0 flex flex-col">
          ...
          return (
            <Link key={item.path} to={item.path}>
              <motion.div whileHover={{ x: 5 }} whileTap={{ scale: 0.98 }}
                className={'flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 ' + (isActive ? 'bg-gradient-to-r from-blue-600 to-purple-600 shadow-lg shadow-blue-500/25' : 'hover:bg-white/10')}>
      ...
      <div className="px-4 py-5 border-t border-gray-700/50">
        <div className="flex items-center justify-between mb-4">
          ...
          <motion.button whileHover={{ scale: 1.1, rotate: 90 }} className="p-1.5 hover:bg-white/10 rounded-lg">
          ...
          {workspaces.length === 0 ? (
            ...
            workspaces.map((w) => (
              <motion.div key={w._id} whileHover={{ x: 3 }}
                className={'px-3 py-2.5 rounded-lg text-sm truncate cursor-pointer transition-all ' + (currentWorkspace?._id === w._id ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30' : 'hover:bg-white/5 text-gray-400')}>
